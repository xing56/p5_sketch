  ...
  (function e(t, n, r) {
      ...
      (function () {
        ...
        function format(str, args) {
          ...
          for (i = 0; i < keys.length; i++) {
            str = str.replace(new RegExp("\\{" + keys[i] + "\\}",
              "gi"), args[keys[i]]);
          }
          return str;
        }
         ...
         */
        CanvasGradient.prototype.addColorStop = function (offset,
          color) {
          var stop = this.__createElement("stop"),
            regex, matches;
          ...
          if (color.indexOf("rgba") !== -1) {
            ...
            stop.setAttribute("stop-color", format(
              "rgb({r},{g},{b})", {
                r: matches[1],
                g: matches[2],
                b: matches[3]
        ...
        ctx = function (o) {
          ...
          this.enableMirroring = options.enableMirroring !==
            undefined ? options.enableMirroring : defaultOptions.enableMirroring;
          ...
          this.__root = document.createElementNS(
            "http://www.w3.org/2000/svg", "svg");
          this.__root.setAttribute("version", 1.1);
          this.__root.setAttribute("xmlns",
          ...
          this.__currentElement = document.createElementNS(
         ...
         * @private
         */
        ctx.prototype.__createElement = function (elementName,
          properties, resetFill) {
          if (typeof properties === "undefined") {
            properties = {};
          }

          var element = document.createElementNS(
              "http://www.w3.org/2000/svg", elementName),
        ...
        ctx.prototype.__applyStyleToCurrentElement = function (type) {
          ...
          for (i = 0; i < keys.length; i++) {
            ...
            if (style.apply) {
              ...
              if (style.apply.indexOf("fill") !== -1 && value instanceof CanvasPattern) {
                ...
                if (value.__ctx) {
                  ...
                  while (value.__ctx.__defs.childNodes.length) {
                    id = value.__ctx.__defs.childNodes[0].getAttribute(
                      "id");
                    this.__ids[id] = id;
                    this.__defs.appendChild(value.__ctx.__defs.childNodes[
                      0]);
                  }
                }
                this.__currentElement.setAttribute("fill", format(
                  "url(#{id})", {
              ...
              else if (style.apply.indexOf(type) !== -1 && style.svg !==
                value) {
                if ((style.svgAttr === "stroke" || style.svgAttr ===
                    "fill") && value.indexOf("rgba") !== -1) {
                  //separate alpha value, since illustrator can't handle it
                  regex =
                    /rgba\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d?\.?\d*)\s*\)/gi;
                  matches = regex.exec(value);
                  this.__currentElement.setAttribute(style.svgAttr,
                    format("rgb({r},{g},{b})", {
                      r: matches[1],
                      g: matches[2],
                      b: matches[3]
        ...
        ctx.prototype.__addTransform = function (t) {
          ...
          var transform = this.__currentElement.getAttribute(
            "transform");
          if (transform) {
            transform += " ";
          }
        ...
        ctx.prototype.__applyCurrentDefaultPath = function () {
          if (this.__currentElement.nodeName === "path") {
            ...
            throw new Error(
              "Attempted to apply path command to node " + this.__currentElement
              .nodeName);
          }
        };
         ...
         */
        ctx.prototype.bezierCurveTo = function (cp1x, cp1y, cp2x,
          cp2y, x, y) {
          this.__currentPosition = {
            x: x,
            y: y
        ...
        var normalize = function (vector) {
          var len = Math.sqrt(vector[0] * vector[0] + vector[1] *
            vector[1]);
        ...
        ctx.prototype.arcTo = function (x1, y1, x2, y2, radius) {
          ...
          var x0 = this.__currentPosition && this.__currentPosition
            .x;
          var y0 = this.__currentPosition && this.__currentPosition
            .y;
          ...
          if (radius < 0) {
            throw new Error("IndexSizeError: The radius provided (" +
              radius + ") is negative.");
          }
          ...
          var cos = (unit_vec_p1_p0[0] * unit_vec_p1_p2[0] +
        ...
        ctx.prototype.stroke = function () {
          if (this.__currentElement.nodeName === "path") {
            this.__currentElement.setAttribute("paint-order",
              "fill stroke markers");
          }
         ...
         * Returns a canvas gradient object that has a reference to it's parent def
         */
        ctx.prototype.createRadialGradient = function (x0, y0, r0, x1,
          y1, r1) {
         ...
         *  Arc command!
         */
        ctx.prototype.arc = function (x, y, radius, startAngle,
          endAngle, counterClockwise) {
          startAngle = startAngle % (2 * Math.PI);
          endAngle = endAngle % (2 * Math.PI);
          if (startAngle === endAngle) {
            //circle time! subtract some of the angle so svg is happy (svg elliptical arc can't draw a full circle)
            endAngle = ((endAngle + (2 * Math.PI)) - 0.001 * (
              counterClockwise ? -1 : 1)) % (2 * Math.PI);
          }
        ...
        ctx.prototype.drawImage = function () {
          ...
          if (image instanceof ctx) {
          ...
          else if (image.nodeName === "CANVAS" || image.nodeName ===
            "IMG") {
            //canvas or image
            ...
            svgImage.setAttributeNS("http://www.w3.org/1999/xlink",
              "xlink:href",
              image.nodeName === "CANVAS" ? image.toDataURL() :
        ...
        ctx.prototype.createPattern = function (image, repetition) {
          ...
          if (image.nodeName === "CANVAS" || image.nodeName ===
            "IMG") {
    ...
    2: [function (require, module, exports) {
      ...
      var Context = function (width, height, options) {
        ...
        this.generations = [
          []
        ]; // used to collect element references for different generations
          ...
          Object.defineProperty(_this, prop, {
            ...
            set: function (val) {
              if (val.indexOf('NaN') > -1) {
                console.warn(
                  "svgcanvas: invalid value for " + prop +
                  ", fail to set it to " + val);
                return;
              }
              _this[key] = val;
            }
        ...
        if (options.debug) {
          ...
          methods.forEach(function (method) {
            ...
            _this[method] = function () {
              var call = method + '(' + Array.prototype.slice.call(
                arguments).join(', ') + ');';
      ...
      Context.prototype.__createElement = function (elementName,
        properties, resetFill) {
        if (!this.__imageSmoothingEnabled) {
          // only shape elements can use the shape-rendering attribute
          if (["circle", "ellipse", "line", "path", "polygon",
              "polyline", "rect"
            ].indexOf(elementName) > -1) {
            properties = properties || {};
            properties["shape-rendering"] = "crispEdges"; // disable anti-aliasing
          }
        }

        var element = C2S.prototype.__createElement.call(this,
          elementName, properties, resetFill);
          ...
          while (ctx.generations.length > 1) {
            ...
            while (count > 0) {
              ...
              if (count === lastCount) {
                ...
                var liveGeneration = ctx.generations[ctx.generations
                  .length - 1];
      ...
      SVGCanvas.prototype.toDataURL = function (type, options) {
        ...
        if (type === "image/jpeg" || type === "image/png") {
          ...
          if (img.complete && img.width > 0 && img.height > 0) {
            ...
            throw new Error(
              'svgcanvas.toDataURL() for jpeg/png is only available in Chrome.'
            );
          }
        }
        throw new Error(
          'Unknown type for SVGCanvas.prototype.toDataURL, please use image/jpeg | image/png | image/svg+xml.'
        );
      };
    ...
    5: [function (require, module, exports) {
      module.exports = function (p5) {
        ...
        p5.prototype.querySVG = function (selector) {
          ...
          return p5.SVGElement.prototype.query.call({
            elt: svg
          }, selector);
        };
        ...
        SVGElement.prototype.unfilter = function (filterName, arg) {
          ...
          filters = filters.reverse().filter(function (filter) {
            if ((filter[0] === filterName) && (filter[1] ===
                arg) && !found) {
              found = true;
              return false;
            }
          ...
          this.attribute('data-p5-svg-filters', JSON.stringify(
            filters));
        ...
        SVGElement.create = function (nodeName, attributes) {
          ...
          var elt = document.createElementNS(
            'http://www.w3.org/2000/svg', nodeName);
        ...
        SVGElement.prototype.matches = function (selector) {
          ...
          var matches = (element.document || element.ownerDocument)
        ...
        p5.prototype.filter = function (operation, value) {
          ...
          if (svg) {
            ...
            nodes.forEach(function (node) {
              if (node !== g.elt && (node.nodeName.toLowerCase() !==
                  'defs')) {
    ...
    8: [function (require, module, exports) {
      module.exports = function (p5) {
        ...
        p5.prototype._makeSVGFrame = function (options) {
          ...
          if (!mine) {
            throw new Error('Fail to getFrame, invalid extension: ' +
              ext + ', please use png | jpeg | jpg | svg.');
          }
        ...
        p5.prototype.saveSVG = function () {
          ...
          this._makeSVGFrame({
            ...
            callback: function (err, frame) {
              p.downloadFile(frame.imageData, frame.filename,
                frame.ext);
            }
          });
        };
        ...
        p5.prototype.saveFrames = function (filename, extension,
        ...
        p5.prototype.save = function () {
          ...
          var useSVG = svg && svg.nodeName && svg.nodeName.toLowerCase() ===
            'svg' && supportedExtensions.indexOf(ext) > -1;
         ...
         * @private
         */
        p5.prototype._svg_get = function (path, successCallback,
          failureCallback) {
          if (path.indexOf('data:') === 0) {
            if (path.indexOf(',') === -1) {
              failureCallback(new Error('Fail to parse dataurl: ' +
                path));
              return;
            }
         ...
         */
        p5.prototype.loadSVG = function (path, successCallback,
          failureCallback) {
    ...
    9: [function (require, module, exports) {
      ...
      module.exports = function (p5) {
        ...
        RendererSVG.prototype._setGCFlag = function (element) {
          ...
          var currentGeneration = that.generations[that.generations
            .length - 1];
          currentGeneration.push(element);
        };
        ...
        RendererSVG.prototype.image = function (img, x, y, w, h) {
          ...
          elt = elt || (img.elt && img.elt.nodeName && (img.elt.nodeName
            .toLowerCase() === 'svg') && img.elt); // SVGElement
          elt = elt || (img.nodeName && (img.nodeName.toLowerCase() ==
            'svg') && img); // <svg>
          if (elt) {
    ...
    10: [function (require, module, exports) {
      module.exports = function (p5) {
        ...
        SVGFilters.apply = function (svgElement, func, arg) {
          ...
          svgElement.attribute('data-p5-svg-filters', JSON.stringify(
            filters));
          ...
          filters = filters.map(function (filter, index) {
            var inGraphics = index === 0 ? 'SourceGraphic' : (
              'result-' + (index - 1));
            var resultGraphics = 'result-' + index;
            return SVGFilters[filter[0]].call(null, inGraphics,
              resultGraphics, filter[1]);
          });

          // get filter id for this element or create one
          var filterid = svgElement.attribute(
            'data-p5-svg-filter-id');
          if (!filterid) {
          ...
          if (!oldfilter) {
            ...
            oldfilter.elt.parentNode.replaceChild(filter.elt,
              oldfilter.elt);
          }
        };
        ...
        SVGFilters.colorMatrix = function (inGraphics, resultGraphics,
          matrix) {
        ...
        SVGFilters.gray = function (inGraphics, resultGraphics) {
          ...
          return SVGFilters.colorMatrix(inGraphics, resultGraphics,
            matrix);
        };

        SVGFilters.threshold = function (inGraphics, resultGraphics,
          val) {
          var elements = [];
          elements.push(SVGFilters.gray(inGraphics, resultGraphics +
            '-tmp'));
          var componentTransfer = SVGElement.create(
            'feComponentTransfer', {
         ...
         */
        SVGFilters.posterize = function (inGraphics, resultGraphics,
          level) {
          ...
          var tableValues = SVGFilters._discreteTableValues(
            function (val) {
        ...
        SVGFilters._blendOffset = function (inGraphics,
          resultGraphics, mode) {
          var elements = [];
          [
            ['left', -1, 0],
        ...
        SVGFilters.erode = function (inGraphics, resultGraphics) {
          return SVGFilters._blendOffset(inGraphics, resultGraphics,
            'darken');
        };
    ...
    11: [function (require, module, exports) {
      ...
      module.exports = function (p5) {
         ...
         *
         */
        p5.prototype.loadGraphics = function (graphics,
          successCallback, failureCallback) {
          if (graphics._renderer.svg) {
            var svg = graphics._renderer.svg;
            var url = SVGCanvas.prototype.toDataURL.call(graphics._renderer
              .elt, 'image/svg+xml');
        ...
        p5.prototype.createCanvas = function (w, h, renderer) {
          ...
          if (renderer === constants.SVG) {
            var c = graphics.elt;
            this._setProperty('_renderer', new p5.RendererSVG(c,
              this, true));
